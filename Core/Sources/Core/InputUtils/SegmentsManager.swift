...
public final class SegmentsManager {
    ...
    private var metadata: ConvertRequestOptions.Metadata {
        if let tag = PackageMetadata.gitTag {
            .init(versionString: "iroha on macOS (\(tag))")
        } else if let commit = PackageMetadata.gitCommit {
            .init(versionString: "iroha on macOS (\(commit.prefix(7)))")
        } else {
            .init(versionString: "iroha on macOS (unknown version)")
        }
    }
    ...
    private func options(
        ...
        return .init(
            ...
            memoryDirectoryURL: self.irohaMemoryDir,
            sharedContainerURL: CompiledUserDictionaryStore.directoryURL(memoryDirectoryURL: self.irohaMemoryDir),
            ...
            typoCorrectionMode: canUseDebugTypoCorrection ? .enabled : .automatic,
    ...
    public var irohaMemoryDir: URL {
        self.applicationDirectoryURL
    }

    public var downloadedInputN5LMDir: URL {
        DebugTypoCorrectionWeights.modelDirectoryURL(
            irohaApplicationSupportDirectoryURL: self.applicationDirectoryURL.deletingLastPathComponent()
        )
    }
    ...
    public func getCurrentMarkedText(inputState: InputState) -> MarkedText {
        ...
        case .composing:
            ...
            return MarkedText(text: [.init(content: text, focus: .none)], selectionRange: NSRange(location: text.count, length: 0))
        case .previewing:
            if let fullCandidate = self.rawCandidates?.mainResults.first,
               self.composingText.isWholeComposingText(composingCount: fullCandidate.composingCount) {
                return MarkedText(
                    text: [.init(content: fullCandidate.text, focus: .none)],
                    selectionRange: NSRange(location: fullCandidate.text.count, length: 0)
                )
            } else {

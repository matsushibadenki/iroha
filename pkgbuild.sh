...
# Suppose we have build/iroha.app
...
pkgbuild --root ${EXPORT_PATH} \
         --component-plist pkg.plist --identifier dev.ensan.inputmethod.iroha \
         --version 0 \
         --install-location /Library/Input\ Methods \
         iroha-tmp.pkg

# Create a distribution file
# productbuild --synthesize --package iroha-tmp.pkg distribution.xml

# Build the final package
productbuild --distribution distribution.xml --package-path . iroha-release.pkg

# Clean up
rm iroha-tmp.pkg

# Sign Pkg
productsign --sign "Developer ID Installer" ./iroha-release.pkg ./iroha-release-signed.pkg
rm iroha-release.pkg
...
xcrun notarytool submit iroha-release-signed.pkg --keychain-profile "Notarytool" --wait

# Staple
xcrun stapler staple iroha-release-signed.pkg

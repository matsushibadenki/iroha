...
if [ "$IGNORE_LINT" = false ]; then
    ...
    then
        ...
        echo "For contributing iroha on macOS, we strongly recommend you to install swiftlint"
...
if [ "$DRY_RUN" = true ]; then
    ...
    echo "  sudo rm -rf /Library/Input\ Methods/iroha.app"
    echo "  sudo cp -r build/archive.xcarchive/Products/Applications/iroha.app /Library/Input\ Methods/"
    echo "  pkill iroha"
    echo "Build completed successfully. Use without --dry-run to actually install."
else
    sudo rm -rf /Library/Input\ Methods/iroha.app
    sudo cp -r build/archive.xcarchive/Products/Applications/iroha.app /Library/Input\ Methods/
    pkill iroha
fi

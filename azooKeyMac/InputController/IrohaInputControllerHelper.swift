...
extension IrohaInputController {
    ...
    @MainActor @objc func performTransformSelectedText(_ sender: Any) {
        ...
        guard let client = self.currentInputClient(reason: "performTransformSelectedText") else {
            return
        }
        let hasSelection = self.safeSelectedRange(client: client, reason: "performTransformSelectedText")?.length ?? 0 > 0
        if hasSelection {
    ...
    @MainActor @objc func validateMenuItem(_ menuItem: NSMenuItem) -> Bool {
        ...
        let aiBackendEnabled = Config.AIBackendPreference().value != .off
        self.updateTransformSelectedTextMenuItemTitle(aiBackendEnabled: aiBackendEnabled)
        return self.canPerformTransformSelectedText(client: self.currentInputClient(reason: "validateMenuItem"))
    }

    func updateTransformSelectedTextMenuItemEnabledState() {
        let aiBackendEnabled = Config.AIBackendPreference().value != .off
        self.updateTransformSelectedTextMenuItemTitle(aiBackendEnabled: aiBackendEnabled)
        self.transformSelectedTextMenuItem.isEnabled = self.canPerformTransformSelectedText(client: self.currentInputClient(reason: "updateTransformSelectedTextMenuItemEnabledState"))
    }
    ...
    private func canPerformTransformSelectedText(client: IMKTextInput?) -> Bool {
        ...
        let hasSelection = self.safeSelectedRange(client: client, reason: "canPerformTransformSelectedText")?.length ?? 0 > 0
    ...
    @objc func openGitHubRepository(_ sender: Any) {
        guard let url = URL(string: "https://github.com/matsushibadenki/iroha") else {
            return
        }
    ...
    func prepareApplicationSupportDirectory() {
        do {
            self.segmentsManager.appendDebugMessage("\(#line): Applicatiion Support Directory Path: \(self.segmentsManager.irohaMemoryDir)")
            try FileManager.default.createDirectory(at: self.segmentsManager.irohaMemoryDir, withIntermediateDirectories: true)

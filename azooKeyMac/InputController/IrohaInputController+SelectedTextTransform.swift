...
extension IrohaInputController {
    ...
    func getContextAroundSelection(client: IMKTextInput, selectedRange: NSRange, contextLength: Int = Constants.defaultContextLength) -> TextContext {
        ...
        let selectedText = self.safeString(client: client, from: selectedRange, actualRange: &actualRange, reason: "getContextAroundSelection.selected") ?? ""

        // Calculate context ranges
        guard let documentLength = self.safeDocumentLength(client: client, reason: "getContextAroundSelection.length") else {
            return TextContext(before: "", selected: selectedText, after: "")
        }
        ...
        let beforeText = (beforeLength > 0) ? (self.safeString(client: client, from: beforeRange, actualRange: &beforeActualRange, reason: "getContextAroundSelection.before") ?? "") : ""

        var afterActualRange = NSRange()
        let afterText = (afterLength > 0) ? (self.safeString(client: client, from: afterRange, actualRange: &afterActualRange, reason: "getContextAroundSelection.after") ?? "") : ""
    ...
    func showPromptInputWindow(initialPrompt: String? = nil) {
        ...
        // Get selected text
        guard let client = self.currentInputClient(reason: "showPromptInputWindow") else {
            self.segmentsManager.appendDebugMessage("showPromptInputWindow: No client available")
            self.isPromptWindowVisible = false
            return
        }

        guard let selectedRange = self.safeSelectedRange(client: client, reason: "showPromptInputWindow") else {
            self.segmentsManager.appendDebugMessage("showPromptInputWindow: Unsafe client; selected text transform skipped")
            self.isPromptWindowVisible = false
            return
        }
        ...
        var actualRange = NSRange()
        guard let selectedText = self.safeString(client: client, from: selectedRange, actualRange: &actualRange, reason: "showPromptInputWindow.selectedText") else {
        ...
        // Get cursor position for window placement. Some Chromium/Electron clients can
        // deadlock on synchronous attributes() calls, so use the shared safe accessor.
        let cursorLocation = self.cursorAnchorLocation(client: client, reason: "showPromptInputWindow") ?? NSEvent.mouseLocation
    ...
    func triggerAiTranslation(initialPrompt: String) -> Bool {
        ...
        guard let client = self.currentInputClient(reason: "triggerAiTranslation") else {
            self.segmentsManager.appendDebugMessage("AI translation ignored: No client available")
            return false
        }
        guard self.safeSelectedRange(client: client, reason: "triggerAiTranslation")?.length ?? 0 > 0 else {
            self.segmentsManager.appendDebugMessage("AI translation ignored: No safe selected text")
            return false
        }
            ...
            do {
                ...
                await MainActor.run {
                    let message = backend == .openAI
                        ...
                        : "transformSelectedText: Using \(backend.rawValue), making request"
    ...
    func replaceSelectedText(with newText: String, usingRange storedRange: NSRange) {
        ...
        guard let client = self.currentInputClient(reason: "replaceSelectedText") else {
            self.segmentsManager.appendDebugMessage("replaceSelectedText: No client available")
            return
        }

        // Check current selection for comparison
        if let currentSelectedRange = self.safeSelectedRange(client: client, reason: "replaceSelectedText.currentSelection") {
            self.segmentsManager.appendDebugMessage("replaceSelectedText: Current selected range: \(currentSelectedRange)")
        } else {
            self.segmentsManager.appendDebugMessage("replaceSelectedText: Current selected range skipped")
        }
    ...
    private func replaceTextUsingIMK(newText: String, storedRange: NSRange) -> Bool {
        ...
        guard let client = self.currentInputClient(reason: "replaceTextUsingIMK") else {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + Constants.replacementVerificationDelay) {
            guard let currentRange = self.safeSelectedRange(client: client, reason: "replaceTextUsingIMK.verify") else {
                self.segmentsManager.appendDebugMessage("replaceTextUsingIMK: Verification skipped for unsafe client")
                return
            }
    ...
    private func replaceTextUsingClipboard(newText: String, storedRange: NSRange) {
        ...
        guard let client = self.currentInputClient(reason: "replaceTextUsingClipboard") else {

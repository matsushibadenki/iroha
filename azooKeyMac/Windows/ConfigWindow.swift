...
struct ConfigWindow: View {
    ...
    @ConfigState private var ollamaApiEndpoint = Config.OllamaApiEndpoint()
    @ConfigState private var mlxSwiftModelName = Config.MLXSwiftModelName()
    ...
    @ConfigState private var autoSwitchUnsupportedAppsToAppleJapanese = Config.AutoSwitchUnsupportedAppsToAppleJapanese()
    ...
    private var irohaApplicationSupportDirectoryURL: URL {
    ...
    private var debugTypoCorrectionModelDirectoryURL: URL {
        DebugTypoCorrectionWeights.modelDirectoryURL(
            irohaApplicationSupportDirectoryURL: self.irohaApplicationSupportDirectoryURL
        )
    }
    ...
    private func openAzooKeyDataDirectoryInFinder() {
        do {
            try FileManager.default.createDirectory(
                at: self.irohaApplicationSupportDirectoryURL,
                withIntermediateDirectories: true
            )
            NSWorkspace.shared.activateFileViewerSelecting([self.irohaApplicationSupportDirectoryURL])
        } catch {
            // no-op
        }
    }
    ...
    func testConnection() async {
        ...
        do {
            ...
            _ = try await AIClient.sendRequest(
                testRequest,
                backend: currentCoreAIBackend,
                apiKey: openAiApiKey.value,
                apiEndpoint: currentAIEndpoint
            )
    ...
    private var currentCoreAIBackend: AIBackend {
        switch aiBackend.value {
    ...
    private var currentAIModelName: String {
        switch aiBackend.value {
        case .ollama:
            return ollamaModelName.value
        case .mlxSwift:
            return mlxSwiftModelName.value
        default:
            return openAiModelName.value.isEmpty ? Config.OpenAiModelName.default : openAiModelName.value
        }
    }

    private var currentAIEndpoint: String {
        switch aiBackend.value {
        case .ollama:
            return ollamaApiEndpoint.value
        default:
            ...
            Section {
                VStack(alignment: .leading) {
                    Picker("いい感じ変換", selection: $aiBackend) {
                        ...
                        Text("Ollama (Gemma E2B)").tag(Config.AIBackendPreference.Value.ollama)
                        Text("MLX Swift").tag(Config.AIBackendPreference.Value.mlxSwift)
                ...
                if aiBackend.value == .ollama {
                    TextField("モデル名", text: $ollamaModelName, prompt: Text("例: gemma4:e2b"))
                    TextField("エンドポイント", text: $ollamaApiEndpoint, prompt: Text("例: http://localhost:11434/api/chat"))
                        .help("Ollamaを起動し、Gemma E2BのOllamaモデルタグ（例: gemma4:e2b）を指定してください。")
                ...
                if aiBackend.value == .mlxSwift {
                    TextField("モデル名", text: $mlxSwiftModelName, prompt: Text("例: Gemma E2B"))
                    Text("MLX Swiftの推論本体は未接続です。開発初期はOllamaを使用し、完成時にCore/MagicConversion/MLXSwiftClient.swiftへ実装を差し替えます。")
                        .font(.caption)
    ...
    private var advancedTabView: some View {
        Form {
            ...
            Section {
                ...
                Toggle("未対応アプリではApple日本語入力へ自動切替", isOn: $autoSwitchUnsupportedAppsToAppleJapanese)
